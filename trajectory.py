...
def plot_animation(x_track, y_track, z_track):
    ...
    max_z = 30.0#max(track[:, 2])
    max_x = 150.0#max(track[:, 0])
    max_y = 10.0#max(track[:, 1])
    ...
    ax.set_xlim3d([0.0, max_x + 50.0])
    ax.set_xlabel('X')
    ax.set_ylim3d([-max_y - 5.0, max_y + 5.0])
    ax.set_ylabel('Y')
    ax.set_zlim3d([0.0, max_z + 10.0])
    ax.set_zlabel('Z')
    ...
    # save gif file
    line_ani.save('most_recent_shot.gif', writer='Pillow', fps=30)
